...
@router.get("/my-todos", status_code=status.HTTP_200_OK)
async def read_all_user_todos(user: user_dependency, db: db_dependency):
...
async def delete_todo(
    ...
    todo_model = (
        db.query(Todos).filter(Todos.id == todo_id).filter(Todos.id == todo_id).first()
    )
